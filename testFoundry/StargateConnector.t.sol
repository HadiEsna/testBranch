...
contract TestStargateConnector is testStarter, MainnetAddresses {
    ...
    function setUp() public {
        ...
        addTokenToNoyaOracle(address(USDC), oracleConnector);
    ...
    function testWithdraw() public {
        ...
        connector.withdrawFromStargatePool(StargateRequest(poolId, 1, 0));
    ...
    function testStakeAndWithdrawFromStake() public {
        ...
        connector.depositIntoStargatePool(StargateRequest(3, 0, lpBalance));
        vm.roll(block.number + 100);
        uint256 rewardBalanceBefore = IERC20(staking.stargate()).balanceOf(address(connector));
        connector.depositIntoStargatePool(StargateRequest(3, _amountDAI, type(uint256).max));
        ...
        connector.withdrawFromStargatePool(StargateRequest(poolId, 0, lpBalanceInStaking / 2));
        connector.withdrawFromStargatePool(StargateRequest(poolId, lpBalanceInStaking, lpBalanceInStaking / 2));
        vm.stopPrank();
    }
}
